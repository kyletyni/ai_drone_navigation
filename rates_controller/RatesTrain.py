...
# model = PPO.load("./models/drl_model_650000_steps", env=vec_env, n_steps=1500, verbose=1)
model = PPO("MlpPolicy", env=vec_env, n_steps=1500, verbose=1)#, policy_kwargs=dict(net_arch=[64, 64], activation_fn=torch.nn.Tanh))
# model = PPO(
#         "MlpPolicy",
#         vec_env,
#         verbose=1,
...
checkpoint_callback = CheckpointCallback(save_freq=20000, save_path='./models/', name_prefix='drl_model')#, verbose=1, save_replay_buffer=False)
